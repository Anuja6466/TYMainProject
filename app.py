...
# IoT 
# ---------------------------
IOT_SHEET_URL = "https://docs.google.com/spreadsheets/d/12vtp-N-gHS-0H7RGnA3IAI1i0qN7tkogNR8jv9TcALs/export?format=csv"

# ================= LIVE DATA =================
def get_iot_data():
    df = pd.read_csv(f"{IOT_SHEET_URL}&nocache={int(time.time())}")
...
def get_daily_average():
    ...
    return {
        "dates": daily_avg["date"].astype(str).tolist(),
        "avg_temp": daily_avg["temperature"].round(2).tolist(),
        "avg_humidity": daily_avg["humidity"].round(2).tolist()
    }
